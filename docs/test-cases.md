...
`PASS` 表示已有执行证据，`FAIL` 表示执行后未满足预期，`NOT EXECUTED` 表示尚未执行；未执行项不以推断替代。
...
测试用例覆盖 Vitest、Playwright、真实集成和人工操作路径，并保留对应的操作级证据。
...
| C-020   | Result Read Ownership 与安全媒体解析      | Unit/Route Contract             | 已认证会话和任务 ID                       | 读取成功任务、无会话任务、RLS 隐藏任务与畸形任务 ID                                   | 只返回当前会话可读任务的 HTTPS Image/Video/Audio；未认证为 401，未知或他人任务为 404    | PASS         | `src/app/api/generations/[taskId]/route.test.ts`；`src/domain/generation/result.test.ts`                 |
| C-021   | Studio Result Workspace 与 Account Result | Controlled Session E2E + Manual | 现有已完成的受控任务                      | 刷新 `/studio?task=<taskId>`；在 Account 查看同一任务；分别验证 Image、Video、Audio   | Studio 与 Account 使用持久化 Result Reference 渲染真实媒体；失败和空结果不伪造媒体      | NOT EXECUTED | 需要可登录的受控用户及已有 Result；不重新调用 fal                                                        |
| C-022   | Result Task Modality Hydration            | Manual UI                       | 当前用户的已完成 Image 与 Audio 任务      | 刷新各自 `/studio?task=<taskId>` 并检查 Tab、Composer、Result 和 Account              | Image 恢复图片模式，Audio 恢复音讯模式；Prompt、Model、媒体和 Task 保持同一关联         | PASS         | 本地人工验收；Image `c3b3d687-9b83-44d1-ae8a-d78847b4c67a`；Audio `90e89602-a3be-4140-85c1-aec790ae61da` |
| C-023   | Result Workspace 操作与布局               | Unit + Manual UI                | 已完成 Image 或 Audio 任务                | 查看 Result Workspace；打开原始结果；点击“复用 Prompt”                                | 结果区按 Prompt、媒体、Model、操作呈现；原始 HTTPS 结果可打开；Prompt 回填对应 Composer | NOT EXECUTED | 需在最新 Production Server 完成 Image、Audio 和移动端人工验收                                            |
| C-024   | Result Workspace 历史栏                   | Manual UI                       | 当前用户有多个已完成或队列中任务          | 打开 `/studio/result?task=<taskId>`；悬停左侧历史项；切换任务                         | 只显示当前用户任务；缩略图/音讯卡、Prompt、创建日期与当前任务一致；无横向溢出           | NOT EXECUTED | 需登录会话完成 Desktop 与 Mobile 人工验收；删除/归档尚未实现                                             |
...
| U-010 | Result Reference HTTPS 过滤、Prompt 映射和 owner-scoped Read API | PASS | `src/domain/generation/result.test.ts`；`src/app/api/generations/[taskId]/route.test.ts` |
| U-011 | Model Registry、模型兼容性和 Provider payload 分发               | PASS | `src/domain/generation/model-registry.test.ts`；`src/integrations/fal/adapter.test.ts`   |
...
| R-008 | Real Integration | Flux Dev、Flux Dev Image-to-Image、Kling V3、MiniMax Speech 2.8 Turbo | PASS | `docs/real-integration/multimodel-2026-08-18.json`；四个 endpoint 返回 COMPLETED 和 HTTPS Result |
